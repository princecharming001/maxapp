...
async def build_agent_system_prompt(
    ...
    chat_prompt += (
        "\n\n## PRODUCT RECOMMENDATIONS — STRICT\n"
        "WHENEVER you mention a specific product, brand, supplement, "
        "skincare item, or anything purchasable, you MUST first call "
        "`recommend_product(module, concern)` and use ONLY the URLs it "
        "returns. The tool filters through the user's known facts "
        "(diet, allergies, sensitivities) so you never recommend a "
        "product they avoid.\n"
        "RULES:\n"
        "1. Pick the best 1–3 products from the CATALOG-VETTED block.\n"
        "2. Quote each `[Name](https://www.amazon.com/<Slug>/dp/<ASIN>)` "
        "markdown link VERBATIM. Do not paraphrase, shorten, or alter "
        "the URL.\n"
        "3. NEVER emit a search URL ('amazon.com/s?k=', 'google.com/"
        "search', etc.) — these are forbidden.\n"
        "4. NEVER invent ASIN paths or guess URLs.\n"
        "5. NEVER use placeholder URLs like `[Name](link)`, `[Name](url)`, "
        "`[Name](here)`, `[Name](TBD)`, `[Name](#)`, `[Name](...)`. If "
        "you do not have a real catalog URL, write the product as PLAIN "
        "TEXT — no brackets, no parentheses, no fake URL.\n"
        "6. If the catalog block is empty, mention the ingredient/"
        "category by name (e.g. 'a 5% minoxidil foam') WITHOUT a link "
        "and tell the user you don't have a vetted link for that yet.\n"
        "7. If the user has dietary or sensitivity facts (vegan, "
        "fragrance allergy, etc.), the catalog has already filtered — "
        "trust it.\n"
    )
    chat_prompt += (
        "\n\n## WEB SEARCH FALLBACK\n"
