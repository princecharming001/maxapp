...
async def build_agent_system_prompt(
    ...
    chat_prompt += (
        ...
        "it returns. The catalog has soft-fallback enabled — it always "
        "returns module-matching products even if the user's exact "
        "concern doesn't tag-match — so DO NOT say things like 'i don't "
        "have specific links', 'i don't have a vetted link', 'i can't "
        "provide links', or anything in that family. If the tool returned "
        "products, link them.\n"
        "RULES:\n"
        ...
        "the URL.\n"
        "2. NEVER emit a search URL ('amazon.com/s?k=', 'google.com/"
        "search', etc.) and NEVER invent ASIN paths or guess URLs.\n"
        "3. NEVER use placeholder URLs like `[Name](link)`, `[Name](url)`, "
        "`[Name](here)`, `[Name](TBD)`, `[Name](#)`, `[Name](...)`.\n"
        "4. If — and ONLY if — `recommend_product` returns the literal "
        "string 'no product recommendations available' (catalog truly "
        "empty for this module), then mention the ingredient/category "
        "by name as plain text without a link. Even then, do not "
        "apologize or say 'i don't have'. Just give the protocol; the "
        "user can search the ingredient name themselves.\n"
