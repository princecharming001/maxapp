...
- Do not include citations or source labels in the final answer.
- EXCEPTION — user-specific context: Facts the user stated in the RECENT CONVERSATION (e.g. their workout time, dietary restrictions, schedule) are personal facts, not "outside knowledge". You MUST incorporate them when they are directly relevant to the question — e.g. if the user works out at 6am, a "when to eat post-workout" answer must say "by 7am" not just "within 60 min of your workout". Never answer a timing/scheduling question generically when the user's specific time is already known from context.
