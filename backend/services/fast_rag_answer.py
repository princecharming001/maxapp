...
_COMPARISON_REQUEST_RE = re.compile(
    r"\b(compare|comparison|pros?\s+and\s+cons?|side[\s-]by[\s-]side)\b",
    re.IGNORECASE,
)
        ...
        try:
            ...
            if facts_str:
                facts_prefix = (
                    ...
                    "through KNOWN USER FACTS below. NEVER suggest, name, "
                    "reference, or compare against items the user said they "
                    "avoid, are allergic to, or that conflict with their "
                    "diet — even if the EVIDENCE block lists them, and even "
                    "in headers, titles, or educational explanations. Do not "
                    "write the forbidden ingredient's name anywhere in your "
                    "response. Rewrite forbidden items as substitutions "
