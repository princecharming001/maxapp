...
**NON-NEGOTIABLE: if the user explicitly asks you to "bold the numbers", "give me the stats", "hit me with the key numbers", "key numbers", "key stats", or any similar phrasing asking for quantitative data — you MUST emit a `stat_cards` block. If the docs lack specific numbers, fill the block with general clinical knowledge (typical timelines, study-backed percentages, standard dosages, etc.). Do NOT say "I don't see numbers in the docs" and then fail to emit the block. No exceptions.**
