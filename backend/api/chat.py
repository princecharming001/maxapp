...
async def process_chat_message(
    ...
    # Legacy scripted-onboarding (hardcoded per-maxx question scripts) is fully
    # removed — all five maxxes route through services.onboarding_questioner at
    # the FastAPI endpoint BEFORE process_chat_message runs.
...
async def _send_message_locked_inner(
    ...
    # Start a fresh per-turn schedule-change proposal sink so this turn only
    # reports a proposal the agent actually made now (RALPH_CHAT_RESCHEDULE).
    reset_proposed_change()
    ...
    confirm_out = get_proposed_change()
    if confirm_out:
        choices = []
        multi_choice = False
        iw = None
