...
- [x] **U8 — `plan_questions` LLM step + fence + fallback, behind `dynamic_questions_enabled`.** _(2026-06-28: `plan_questions` (sync, run off-thread by caller) builds slots payload, calls `_complete_json` (isolated provider boundary), parses strict JSON → QuestionPlan, FENCES (drop hallucinated/known, dedup, importance re-sort, cap at gap len), memoizes per (maxx,gap,brief) for TTL. Any failure/empty → None. test_plan_questions_fence + _fallback green. Caveat flagged below: model pinning + cache_control not yet wired.)_
...
- [x] **U10 — plan_dirty chat-update hook.** _(2026-06-28: `mark_onboarding_plan_dirty(user_id,db)` (one ctx read + conditional write, no LLM) called after the Tier-0 chat-fact persist when `dynamic_questions_enabled`; `recompute_dirty_plan` drops now-known queued slots + re-anchors idx + clears flag, wired into the plan continue-path. Chose a DETERMINISTIC re-filter over a fresh LLM recompute (cheaper, satisfies "drop now-known"). test_plan_dirty_recompute + mark-dirty test green (12).)_
...
- [x] **U12 — Enrich the 6 docs with real `info_schema` (cross-Max `satisfied_by`).** _(2026-06-28: explicit `info_schema` added to fitmax + heightmax covering ALL required fields, with the shared canonical `sleep_hours` slot + `span(sleep_time,wake_time)` derive (verified: derives 8.0 from global times alone → never re-asked) and tuned importance. Other 4 docs stay on clean auto-derive (criterion 7 allows it; same-id fields like outdoor_exposure/skin_type already dedup cross-Max via facts aliases). Vocab-mismatched equivalences (workout_frequency/training_status/experience_level/equipment vs equipment_access) deliberately NOT guessed — flagged below. test_all_docs_compile_no_dead_required green; 33 passed (only pre-existing collision red).)_
...
- [x] **U13 — Idempotent backfill job (optional, guarded, not auto-run).** _(2026-06-28: NEW `backend/scripts/backfill_onboarding_facts.py` — pure `onboarding_to_fact_calls` maps each onboarding answer that backs a real slot to a keyed `remember_fact(key="onboarding.<f>", source="onboarding", conf=0.85)`; keyed dedupe makes a replay idempotent. CLI `--dry-run`/`--user`; NOT imported by the app or the loop. test_backfill_idempotent green (15). Prod-run flagged below.)_
...
- **[U13] Production backfill run (NOT executed).** `scripts/backfill_onboarding_facts.py` is shipped + unit-tested but never auto-run (the local backend points at the PROD Supabase DB, so running it touches prod data). DECISION NEEDED: when/whether to run the one-time backfill against prod (it is idempotent + safe — keyed `onboarding.*` facts, source rank "onboarding" never clobbers stronger chat facts), and whether to gate it behind a maintenance window. `onboarding:<id>` aliases already resolve onboarding answers live, so backfill is an enhancement (profile/facts uniformity), not a prerequisite for cross-Max dedup.
- **[U12] Cross-Max slot taxonomy / value equivalences (NOT guessed).** Several maxes ask conceptually-overlapping but value-INCOMPATIBLE questions: fitmax `experience_level` {beginner/intermediate/advanced} + `days_per_week` (int) vs bonemax `workout_frequency` {none/light/moderate/heavy} vs heightmax `training_status` {yes_regular/yes_some/no}; and fitmax `equipment` {full_gym/home_barbell/...} vs heightmax `equipment_access` {none/pullup_bar/inversion_table/full_setup}. Aliasing these cross-Max would prefill an out-of-vocab enum value (silently mis-routing the DSL and suppressing the question) — so they are LEFT as independent slots. DECISION NEEDED: define canonical activity/equipment slots + a deterministic value-mapping (a new `derive`/normalizer, e.g. `map(experience_level -> workout_frequency)`) so these genuinely dedup. Until then they re-ask per-Max (today's behavior; safe). Safe same-id overlaps (sleep_hours, age, outdoor_exposure, skin/hair/scalp facts) already dedup.
- **[U8] LLM provider/model pinning + prompt-cache wiring (provisional).** `plan_questions._complete_json` currently routes through `lc_providers.get_sync_json_llm` (max_tokens=1500), which uses the `LLM_PROVIDER`-configured model — it does NOT yet pin `settings.dynamic_questions_model` (default `claude-haiku-4-5`) or attach Anthropic `cache_control` on the static system prefix. The system prompt is already isolated as the cacheable prefix, and the path is default-OFF (`dynamic_questions_enabled=False`), so this is non-blocking. DECISION NEEDED: confirm provider (haiku via `claude` provider vs. the HF default), whether to honor `dynamic_questions_model` by adding a model arg to `get_sync_json_llm` (or switch to `claude_service.simple_completion`), wire ephemeral prompt-caching, and set the per-intake cost/token ceiling.
