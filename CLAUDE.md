...
- **Marketplace** `backend/api/marketplace.py` + `backend/data/product_catalog.yaml`. Native maxes are **included** in the subscription (tier slot caps — ChadLite/basic = 2, Chad/premium = 3 — with a 7-day swap lock); creator courses are paid via Stripe. `product_catalog.yaml` is the **only** source for product recommendations — `services/link_validator.py` rejects any URL not in it.
- **Payments** Stripe + Apple IAP (StoreKit). `services/apple_iap_service.py` verifies via the App Store Server API. Tiers: `basic` (ChadLite) / `premium` (Chad).
