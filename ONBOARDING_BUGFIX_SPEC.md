...
- [x] **T6 — Prefetch chat-history on the Start-schedule CTA.** _(2026-06-28: extracted shared `fetchChatHistory(conversationId)` in useAppQueries.ts; hook delegates to it. Both CTAs (MaxxDetailScreen onPress + MaxDetailScreen goToChat) call `queryClient.prefetchQuery({queryKey: queryKeys.chatHistory, queryFn: () => fetchChatHistory(null), staleTime: 60_000})` immediately before navigate — byte-identical key/fn to `useChatHistoryQuery(null)` so the screen consumes the warm cache. historyReady gate untouched. tsc clean for the 3 files; only glass/* baseline. Manual throttled-sim: sim-unverified.)_ (ROOT-CAUSE fix for LAG.) Extract `useChatHistoryQuery`'s null-id `queryFn` in `mobile/hooks/useAppQueries.ts:144-166` into a shared exported fn (e.g. `fetchChatHistory`) so the screen and the prefetch use **byte-identical** `queryFn` + `queryKey`. In both CTAs — `mobile/screens/courses/MaxxDetailScreen.tsx` onPress (403-409) and `mobile/screens/marketplace/MaxDetailScreen.tsx` `goToChat` (469-474) — call `queryClient.prefetchQuery({ queryKey: queryKeys.chatHistory, queryFn: fetchChatHistory, staleTime: 60_000 })` **immediately before** `navigation.navigate(...)`. By mount the history query is already in-flight/cached, so `historyReady` flips ~immediately, the full-screen spinner window (1310-1313) collapses, and the first-question POST fires right away. Ensure the prefetch `queryKey`/`queryFn` exactly match the null-id branch (so the screen consumes the cache, not a second GET). **Do NOT relax the `historyReady` gate.**
...
- [x] **T7 — Render optimistic bubble + typing shimmer for the `initSchedule` entry instead of full-screen spinner.** _(2026-06-28: full-screen ActivityIndicator gate now also requires `!route.params?.initSchedule` — onboarding entry renders the normal list (optimistic user line + ThinkingShimmer from T6) instead of a blank spinner; normal cold Chat-tab entry still shows the spinner. historyReady send-gating untouched. tsc clean; only glass/* baseline. sim-unverified.)_ (COMPLEMENTARY LAG win; additive to T6.) In `mobile/screens/chat/MaxChatScreen.tsx`, gate the full-screen `<ActivityIndicator>` at 1310-1313 to NOT apply when `route.params.initSchedule` is present — for the onboarding entry, render the normal list (which, once T6 lands, shows the optimistic user line + `ThinkingShimmer` immediately). Keep the spinner for a normal cold Chat-tab entry. Do NOT alter the `historyReady` gating of the send itself.
...
- [x] **T8 — Final end-to-end verification of both fixes + fallback intact.** _(2026-06-28: full `pytest tests/` = 638 passed / 6 failed — all 6 are the documented pre-existing baseline (chat_routing knowledge-route, max_doc collision + entitlement-402, fast_rag-citations, p0_security x2 — the latter 3 confirmed pre-existing at the pre-U1 baseline commit during DYNAMIC_ONBOARDING U14); passed count rose 635→638 (the 3 new T5 tests), failing SET unchanged → zero new reds. Targeted onboarding 36 (≥33). Mobile tsc clean for all touched files (only 5 glass/* baseline). Flags-OFF smoke = `False False claude-haiku-4-5`. Manual sim e2e: sim-unverified (guardrail 7); both fixes structurally verified by tests + code/tsc.)_ Backend full suite + targeted onboarding files; flags-OFF smoke. Mobile typecheck of all touched files. Manual end-to-end (sim, `com.cannon.mobile`): fresh paid user → Explore → a doc-driven Max → Start schedule; confirm (a) LAG: instant entry into a typing state, no long blank spinner; (b) ERROR: with a fault injected, the chat continues via the agent fallback (or shows the friendly message) instead of a dead "Internal server error" bubble. Confirm answer persistence: `merge_context` writes the answer before any raising line, and the degrade re-enters the agent/legacy flow. Maestro optional (sim-unverified OK): add `assertNotVisible "Internal server error"` after the answer step.
...
2026-06-28 T6 — Shared `fetchChatHistory` + prefetch on both Start-schedule CTAs (byte-identical key/fn to the hook). historyReady gate untouched. tsc clean for the 3 files (only glass/* baseline). LAG root-cause fix; sim-unverified.
2026-06-28 T7 — Full-screen spinner gate now excludes `initSchedule` entries (render list + optimistic typing instead). Normal cold Chat-tab spinner kept. tsc clean (only glass/* baseline). sim-unverified.
2026-06-28 T8 — E2E verify: backend 638 passed / 6 pre-existing reds (passed 635→638, failing set unchanged = zero new reds), targeted onboarding 36, mobile tsc clean for touched files (only glass/* baseline), flags-OFF smoke False/False/claude-haiku-4-5. Manual sim e2e sim-unverified. ALL UNITS DONE.

## Full pre-existing red baseline (NOT caused by this work — verified at the pre-U1 commit)
- `tests/test_chat_routing.py::test_process_chat_message_routes_knowledge_away_from_context_and_agent`
- `tests/test_entitlement_regression.py::TestSubscriptionExpiry::test_expired_subscription_raises_402`
- `tests/test_fast_rag_and_retriever.py::test_answer_from_chunks_preserves_clean_citations`
- `tests/test_max_doc_pipeline.py::test_validator_fixes_collisions_and_titles`
- `tests/test_p0_security.py::TestScanUserBypass::test_scan_user_skips_require_paid_user`
- `tests/test_p0_security.py::TestScanUserBypass::test_non_paid_non_scan_user_gets_402`
- plus 2 collection errors from DB-connect scripts (`scripts/test_db_connection.py`, `test_supabase.py`).
- Mobile tsc: 5 pre-existing errors in `components/glass/GlassButton.tsx` + `GlassCard.tsx` (Tamagui prop typing).
